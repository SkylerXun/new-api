...
type User struct {
	...
	BillingCurveMultiplier    float64                    `json:"billing_curve_multiplier" gorm:"-:all"`
	MonthlyDiscountMultiplier float64                    `json:"monthly_discount_multiplier" gorm:"-:all"`
}
...
// BindInviter assigns an inviter exactly once and increments the inviter's
// count atomically. This is reserved for administrator workflows; self-service
// registration continues to bind during user creation.
func BindInviter(userID, inviterID int) error {
	if userID <= 0 || inviterID <= 0 || userID == inviterID {
		return errors.New("invalid inviter")
	}
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		result := tx.Model(&User{}).
			Where("id = ? AND (inviter_id = 0 OR inviter_id IS NULL)", userID).
			Updates(map[string]interface{}{"inviter_id": inviterID})
		if result.Error != nil {
			return result.Error
		}
		if result.RowsAffected == 0 {
...
func (user *User) finishInsert(inviterId int) {
	...
	if inviterId != 0 {
		if err := inviteUser(inviterId); err != nil {
			common.SysError(fmt.Sprintf("failed to record invitation for inviter %d: %s", inviterId, err.Error()))
		}
	}
}
